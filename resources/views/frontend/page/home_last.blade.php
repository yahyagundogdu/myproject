...
@section('content')
    ...
    @include('frontend.include.header_bottom')
        ...
        <div class="container">
            <div class="about__our-misson wow fadeInLeft" data-wow-duration="1s">
                ...
                <div class="about__our-misson-text">
                    ...
                    <h1 class="title">{{ $page->misyon }}</h1>
                    <p class="text">{{ $page->misyon_icerik }}
                    </p>
                </div>

                <div class="about__our-misson-img">
                    <img src="{{ Storage::url($page->resim_1) }}">
                </div>
            ...
            <div class="about__text wow fadeInRight" data-wow-duration="1s">
                <h4>{{ $page->baslik_1 }}</h4>
                <h1 class="section-title" style="line-height:70px">{{ $page->buyuk_baslik_1 }}</span> </h1>
                <h2>{{ $page->baslik_3 }}</h2>
                <p style="text-align:justify;">{{ $page->aciklama_1 }}</p>
                <div class="about__text-signature flex">
                    <img src="/frontend/images/png/signature.png" alt="signature" style="width: 300px">
                </div>
            </div>
        </div>
    </section>
    <section class="why-choose-us" id="why-us">
        <div class="why-choose-us__bg">
            ...
            <img src="{{ Storage::url($page->resim_2) }}" alt="">
        </div>
        <div class="container">
            <h5>{{ $page->baslik_4 }}</h5>
            <h1 class="section-title"><span>{{ $page->buyuk_baslik_2 }}</span></h1>
            <div class="difference">
                @foreach ($page_cart as $value)
                    @php
                        $json_decode = null;
                        $json_decode[] = json_decode($value->data, true);
                    @endphp
                    <div class="difference__item wow fadeInLeft" data-wow-duration="1s">
                        <h1 class="difference__item-number">{{ $loop->iteration }}</h1>
                        <div class="difference__item-content">
                            <h1 class="difference__item-title">{{ $json_decode[0][0]['data_value'] }}</h1>
                            <p class="difference__item-text" style="text-align:justify;">{{ $json_decode[0][1]['data_value'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <section class="appointments" id="departments">
        <div class="appointments__bg">
            <div class="icon">
                <img src="/frontend/images/png-shapes/appointments-shape.png">
            </div>
        </div>
        <div class="container">
            <div class="online-appointments">
                <h1 class="section-title"><span>{{ $page->buyuk_baslik_3 }}</span></h1>
                <p class="appointments-text">{{ $page->aciklama_2 }}</p>
            </div>

            <div class="company-numbers">

                <div class="company-numbers__item">
                    <h2 class="number">{{ $page->sayi_1 }}</h2>
                    <div class="content">
                        <h3 class="title">{{ $page->sayi_1_baslik }}</h3>
                        <p class="text">{{ $page->sayi_1_aciklama }}</p>
                    </div>
                </div>

                <div class="company-numbers__item">
                    <h2 class="number">{{ $page->sayi_2 }}</h2>
                    <div class="content">
                        <h3 class="title">{{ $page->sayi_2_baslik }}</h3>
                        <p class="text">{{ $page->sayi_2_aciklama }}</p>
                    </div>
                </div>
    ...
    <section class="testimonials">
        <div class="testimonials__img">
            <img src="{{Storage::url($page->yorumlar) }}">
        </div>
        <div class="container testimonials__content wow fadeInRight" data-wow-duration="1s">
            <h1 class="section-title"><span>{{ $page->yorumlar_basligi }}</span></h1>
            <div class="testimonials__content-carousel owl-carousel owl-theme">
                @foreach ($yorumlar as $yorum_value)
                    @php
                        $json_decode = null;
                        $json_decode[] = json_decode($yorum_value->data, true);

                    @endphp
                    <div class="testimonials__content-carousel-item">
                        <q>{{ $json_decode[0][1]['data_value'] }}</q>
                        <h2>{{ $json_decode[0][0]['data_value'] }}</h2>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <section class="latest-news">
        ...
        <div class="container">
            <h1 class="section-title"><span>{{ $page->baslik_6 }}</span></h1>

            <div class="latest-news__carousel owl-carousel owl-theme">
                @isset($blog)
                    @foreach ($blog as $item)
                        <a href="{{ route('blogPage', $item->slug) }}" class="latest-news__item">
                            <div class="inner-block">
                                ...
                                <p class="text">
                                    {{ Str::limit(strip_tags(html_entity_decode($item->body, ENT_COMPAT, 'UTF-8')), 200) }}
                                </p>
